# rag/core/domain_scope.py
"""Clasificación de alcance del dominio (costero/jurídico) y esquema
``EnrichedQuery``.

Extraído mecánicamente de ``query_enricher.py`` (depuración organizacional
del módulo RAG): este archivo reúne el esquema de salida estructurada del
enriquecedor (``EnrichedQuery``, ``QueryRoute``, ``DocumentType``), su
normalización (``_normalize_expanded_query``), y el clasificador
determinista de alcance organizado en siete familias de señal, junto con
``_fallback()`` y ``_apply_domain_guard()``, que son quienes lo consumen.

``query_enricher.py`` conserva la construcción del prompt del LLM
(``_build_human_body``, ``_build_prompt``), el parseo de su respuesta JSON
(``_parse_json_response``, con su ``_JSON_FENCE_RE`` — que NO se movió) y
la API pública (``enrich_query``/``enrich_query_async``); reexporta desde
aquí ``EnrichedQuery``, ``QueryRoute``, ``DocumentType``,
``_normalize_expanded_query``, ``_fallback``, ``_apply_domain_guard`` y
``_has_legal_signal`` porque varios tests los importan directamente desde
``rag.core.query_enricher`` (p. ej. ``tests/unit/test_query_analysis.py``,
``tests/unit/test_expanded_query_length.py``, ``tests/unit/test_core_generator.py``,
``tests/unit/test_agent_graph.py``) y ``rag.core.agent`` importa
``QueryRoute``/``EnrichedQuery`` desde ``.query_enricher``.
"""
...
#   6. conversación o meta-pregunta       -> _is_meta_question
...
# --- Familia #6: conversación o meta-pregunta -------------------------------
